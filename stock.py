...
if selected_ticker:
    ...
    with tab3:
        ...
        if not history_df.empty:
            # 최근 5회 승률
            res = history_df[history_df['적중여부'].isin(["⭕ 적중", "❌ 실패"])]
            if not res.empty:
                recent = res.head(5)
                st.markdown(f"### 🔥 최근 5회 승률: {(len(recent[recent['적중여부']=='⭕ 적중'])/len(recent)*100):.1f}%")
            
            # 국가별 분리 및 링크 연동
            history_df['시장'] = history_df['종목코드'].apply(lambda x: "한국" if (x.endswith('.KS') or x.endswith('.KQ')) else "미국/해외")
            history_df['링크주소'] = history_df.apply(lambda r: f"/?jump_tk={r['종목코드']}&jump_name={urllib.parse.quote(r['종목명'])}", axis=1)
            
            for m in ["한국", "미국/해외"]:
                st.subheader(f"{m} 예측 기록")
                st.dataframe(history_df[history_df['시장']==m][['예측일자', '종목코드', '링크주소', 'AI예측', '적중여부']], column_config={"링크주소": st.column_config.LinkColumn("종목명", display_text=r"^/\?jump_tk=.+&jump_name=(.+)$")}, use_container_width=True)
